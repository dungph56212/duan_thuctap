    ...
    .action-buttons .btn {
        ...
        margin-bottom: 5px;
            ...
            <div class="bulk-actions" id="bulkActions">
                <form method="POST" action="?ctl=lienhe&act=bulk" id="bulkForm">
                        ...
                        <?php else: ?>
                            <?php foreach ($ds_lienhe as $lienhe): ?>
                                <tr class="contact-row <?= !$lienhe['is_read'] ? 'unread-row' : '' ?>">
                                    ...
                                    <td>
                                        <strong><?= htmlspecialchars($lienhe['subject'] ?? 'Không có tiêu đề') ?></strong>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <span class="status-badge status-<?= $lienhe['status'] ?? 'pending' ?>">
                                            <?php 
                                                $status = $lienhe['status'] ?? 'pending';
                                                switch($status) {
                                    ...
                                    <td>
                                        <span class="priority-badge priority-<?= $lienhe['priority'] ?? 'normal' ?>">
                                            <?php 
                                                $priority = $lienhe['priority'] ?? 'normal';
                                                switch($priority) {
                                                    case 'low': echo 'Thấp'; break;
                                                    case 'normal': echo 'Bình thường'; break;
                                                    case 'high': echo 'Cao'; break;
                                                    case 'urgent': echo 'Khẩn cấp'; break;
                                    ...
                                    <td>
                                        <div class="action-buttons">
                                            <a href="?ctl=lienhe&act=view&id=<?= $lienhe['id'] ?>" 
                                               class="btn btn-info btn-sm" title="Xem chi tiết">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            
                                            <?php if (($lienhe['status'] ?? 'pending') !== 'replied'): ?>
                                            ...
                                            <button type="button" class="btn btn-warning btn-sm" 
                                                    onclick="openStatusModal(<?= $lienhe['id'] ?>, '<?= $lienhe['status'] ?? 'pending' ?>', '<?= $lienhe['priority'] ?? 'normal' ?>')" 
                                                    title="Cập nhật">
                                                <i class="fas fa-edit"></i>
                                            </button>
                                            
                                            <a href="?ctl=lienhe&act=delete&id=<?= $lienhe['id'] ?>" 
                ...
                <?php if (($total_pages ?? 1) > 1): ?>
                    <nav class="mt-4">
                        ...
                        <div class="text-center text-muted">
                            Hiển thị <?= (($current_page - 1) * 10) + 1 ?> - <?= min($current_page * 10, $total_records ?? 0) ?> 
                            trong tổng số <?= $total_records ?? 0 ?> liên hệ
                        </div>
                    </nav>
                <?php endif; ?>
            </div>
...
<div class="modal fade" id="replyModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form method="POST" action="?ctl=lienhe&act=reply">
...
<div class="modal fade" id="statusModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form method="POST" action="?ctl=lienhe&act=updateStatus">
                <div class="modal-body">
...
setInterval(function() {
    fetch('?ctl=lienhe&act=getUnreadCount')
