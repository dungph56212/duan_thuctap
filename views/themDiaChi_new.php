                                        ...
                                        <div class="tab-pane fade show active" role="tabpanel">
                                            <div class="myaccount-content">
                                                ...
                                                <div class="account-details-form">
                                                    <form action="<?= BASE_URL ?>?act=them-dia-chi" method="post">
                                                        <div class="row">
                                                            <div class="col-lg-6">
                                                        ...
                                                        <div class="single-input-item">
                                                            ...
                                                            <textarea id="dia_chi_chi_tiet" name="dia_chi_chi_tiet" rows="3" 
                                                                      placeholder="Nhập địa chỉ chi tiết (số nhà, tên đường...)" required><?= isset($_POST['dia_chi_chi_tiet']) ? htmlspecialchars($_POST['dia_chi_chi_tiet']) : '' ?></textarea>
                                                        </div>

                                                        <div class="single-input-item">
                                                            <div class="form-check">
                                                                <input class="form-check-input" type="checkbox" id="is_default" name="is_default" value="1"
                                                                       <?= (isset($_POST['is_default']) && $_POST['is_default']) ? 'checked' : '' ?>>
...
$(document).ready(function() {
    ...
    $.getJSON('https://provinces.open-api.vn/api/p/', function(provinces) {
        provinces.forEach(function(province) {
            $("#tinh_thanh").append(`<option value="${province.name}">${province.name}</option>`);
        });
    });
            ...
            if (selectedProvince) {
                ...
                $.getJSON(`https://provinces.open-api.vn/api/p/${selectedProvince.code}?depth=2`, function(provinceData) {
                    ...
                    provinceData.districts.forEach(function(district) {
                        $("#quan_huyen").append(`<option value="${district.name}">${district.name}</option>`);
                    });
                    ...
                    if (selectedDistrict) {
                        ...
                        $.getJSON(`https://provinces.open-api.vn/api/d/${selectedDistrict.code}?depth=2`, function(districtData) {
                            ...
                            districtData.wards.forEach(function(ward) {
                                $("#phuong_xa").append(`<option value="${ward.name}">${ward.name}</option>`);
                            });
                        });
                    }
                });
...
<!-- Custom CSS for Add Address Page -->
<style>
.required::after {
    content: " *";
    color: red;
